...
- [Transaction Flows](#transaction-flows)
  ...
  - [Budget Alert Flow](#budget-alert-flow)
...
participant "Budget Service" as BS
...
AE -> BS: CheckBudgetLimits
activate BS

BS -> DB: FetchCurrentSpending
activate DB
DB --> BS: Spending Data
deactivate DB

BS -> AS: EvaluateAlerts
activate AS

AS -> ES: Store BudgetAlertEvent
ES --> AS: Event Stored

AS -> EB: Publish BudgetAlertEvent
...
EB -> QM: Handle BudgetAlertEvent
activate QM
QM -> DB: Update Alert Status
DB --> QM: Updated
deactivate QM

EB -> NS: Handle BudgetAlertEvent
activate NS
NS -> NS: Generate Notification
NS -> NS: Send Alert
deactivate NS
@enduml
```
